...
#define XXTEA_KEY "ZHCLOVEWRH1314"
...
void Util::init()
{
	...
	string str = Util::fileUtils->getStringFromFile("image/icon.png");
	xxtea_long ret_length;
	unsigned char *encrypted = xxtea_encrypt((unsigned char *)str.c_str(), str.length(), (unsigned char *)XXTEA_KEY, strlen(XXTEA_KEY), &ret_length);
	ValueMap vm;
	string encryptedStr((const char*)encrypted);
	vm["data"] = encryptedStr;
	Util::fileUtils->writeToFile(vm, Util::fileUtils->getWritablePath() + "data.plist");
